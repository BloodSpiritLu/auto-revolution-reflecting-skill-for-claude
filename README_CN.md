...
### 当前策略
- **冲突即更新**：新规则与旧规则冲突时，直接更新旧规则（最简单有效）

### 计划中（尚未实现）

| 功能 | 描述 | 复杂度 |
|------|------|--------|
| **定期审查** | 添加 `/review` 命令，手动触发规则审查 | 低 |
| **置信度衰减** | 高→中→低→删除，长期未验证的规则自动降级 | 中 |
| **时间衰减** | 记录添加时间，超过 N 个月未强化的规则提示清理 | 中 |
| **容量限制** | 每类别限制 N 条，超出时提示选择删除 | 低 |
| **使用频率** | 跟踪规则被"触发"的次数，清理低频规则 | 高 |

### 触发时机选项（未来）
- 时间驱动：每隔 N 天/周提醒
- 事件驱动：每 N 次 reflect 后触发
- 手动触发：用户主动调用 `/review`
